...
"SELECT projects.title, SUM(pledges.amount) - projects.funding_goal FROM pledges INNER JOIN projects on projects.id = pledges.project_id GROUP BY projects.title HAVING (SUM(pledges.amount) - projects.funding_goal) >= 0;"
end
...
"SELECT projects.category, SUM(pledges.amount) FROM pledges INNER JOIN projects ON projects.id = pledges.project_id GROUP BY projects.category HAVING projects.category = 'music';"
end
