...
"SELECT projects.title, SUM(pledges.amount) - SUM(projects.funding_goal) FROM pledges INNER JOIN projects on projects.id = pledges.project_id GROUP BY projects.title HAVING (SUM(pledges.amount) - SUM(projects.funding_goal)) > 0"
end
